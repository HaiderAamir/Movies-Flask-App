...
app.config['POSTER_UPLOAD'] = para['poster_location']
app.config['MOVIE_UPLOAD'] = para['movies_location']
    ...
    if (request.method == "POST"):
        ...
        if recordname:
            ...
            if(password1==password2):
                msg = Message('Welcome to MovieVerse', sender = para['gmail-user'], recipients = [email1])
...
def uploadmovie():
    if ('user' in session and session['user'] == para['admin_user']):
        ...
        if request.method == 'POST':
            ...
            session['user'] = para['admin_user']
            session['password'] = para['admin_password']
...
def recover():
    if request.method == 'POST':
        ...
        if data:
             msg = Message('Recover Password', sender = para['gmail-user'], recipients = [data.email])
...
def regusers(id):
    if ('user' in session and session['user'] == para['admin_user']):
        ...
        if checkdata:
            ...
            db.session.commit()
            msg = Message('MoviesVerse', sender = para['gmail-user'], recipients = [reg.email])
...
def sendendmail():
    if ('user' in session and session['user'] == para['admin_user']):
        ...
        for users in datamail:
            msg = Message('MoviesVerse', sender = para['gmail-user'], recipients = [users.email])
